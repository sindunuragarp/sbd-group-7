...
object Bacon {
	...
	final val bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("sparkLog.txt"), "UTF-8"))
	...
	def getActor2MoviesList (s: String, g: Boolean) : ((String, Boolean), Array[String]) = {
		...
		val actor = data(0)
		val movie_list1 = data.slice(1, data.length)
  ...
  ////
    ...
    val actor2ListOfMoviesM = inputM
      .map(x => getActor2MoviesList(x._2.toString, g = true))
      .filter(x => x != null)

    val actor2ListOfMoviesF = inputF
      .map(x => getActor2MoviesList(x._2.toString, g = false))
      .filter(x => x != null)

    val actor2ListOfMovies1 = actor2ListOfMoviesM.union(actor2ListOfMoviesF)
      .zipWithIndex
      .map{case(((actor, gender), movielist), i) =>
        if (gender)
          ((actor, movielist), i)
        else
          ((actor, movielist), -1*(i+1))
      }
    ...
    // (actor_id, actor)
    val actorID2actor = actor2ListOfMovies1.map{
      case((actor, ml), id) => (id, actor)
    }
    ...
    val perc_female_actors = (num_female_actors * 100) / num_of_actors
		...
		fout.write("Total number of actors = " + num_of_actors.toString + ", out of which " + num_male_actors + " (" + perc_male_actors + "%) are males while " + num_female_actors + " (" + perc_female_actors + "%) are females.\n")
    ...
    var countActors: Long = 0
		...
		for (dist <- 0 until Distance) {
			...
			fout.write("There are " + countListActors(dist) + " (" +  f"$perc_actors%.1f" + "%) actors and " + countListActresses(dist) + " (" + f"$perc_actresses%.1f" + "%) actresses at distance " + (dist + 1) + "\n")
		}

    ////

		fout.write("\nTotal number of actors from distance 1 to 6 = " + (countActors + countActresses) + ", ratio = " + ((countActors + countActresses).toFloat / num_of_actors) + "\n")
		fout.write("Total number of male actors from distance 1 to 6 = " + countActors + ", ratio = " + (countActors.toFloat / num_male_actors) + "\n" )
		fout.write("Total number of female actors (actresses) from distance 1 to 6 = " + countActresses + ", ratio = " + (countActresses.toFloat / num_female_actors) + "\n")

    ////

		fout.write( "\nList of male actors at distance 6:\n" )
    var index = 0
		for (x <- x6.filter(x => x._2)) {
			index += 1

    ...
    def process_records(self, stream):
        try:
            for record in ArchiveIterator(stream):
                ...
                try:
                    for num in self.find_phone_numbers(record.content_stream()):
                        yield (num, uri)
...
if __name__ == "__main__":
    ...
    pn = PhoneNumbers(conf.input, conf.output, conf.name, partitions=conf.partitions)
    pn.run()
